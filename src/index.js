...
// TO AVOID REPETITION
async function mesageWinnerBlocks(characterName, block) {
    console.log(chalk.green.bold(`\n🎉 O vencedor da rodada de ${block} é ${characterName}! ! 🎉`));
}
    ...
    for (let round = 1; round <= 5; round++) {
        ...
        if (block === "RETA") {
            ...
            if (totalSkill1 > totalSkill2) {
                mesageWinnerBlocks(character1.NOME, block);
                character1.PONTOS += 1;
            } else if (totalSkill2 > totalSkill1) {
                mesageWinnerBlocks(character2.NOME, block);
                character2.PONTOS += 1;
            } else {
                console.log(chalk.bgGray(`\nNenhum jogador marcou pontos nesta rodada.`));
            }
            ...
            if (powerResult1 > powerResult2) {
                ...
                if (powerResult1 > powerResult2 && character2.PONTOS > 0) {
                    console.log(chalk.redBright(`\n${character2.NOME} perdeu 1 ponto por ter sido derrotado no confronto!`));
                }
            } else if (powerResult2 > powerResult1) {
                mesageWinnerBlocks(character2.NOME, block);
                character2.PONTOS += 1;
                if (powerResult2 > powerResult1 && character1.PONTOS > 0) {
                    console.log(chalk.redBright(`\n${character1.NOME} perdeu 1 ponto por ter sido derrotado no confronto! 🐢`));
                }
            } else {
                console.log(chalk.bgGray(`\nNenhum jogador marcou pontos nesta rodada.`));
            }
...
async function declareWinner(character1, character2) {
    ...
    if (character1.PONTOS > character2.PONTOS) {
        console.log(chalk.bgCyan.greenBright.bold (`🎉 O grande vencedor é ${character1.NOME}! Parabéns! 🎉`));
    } else if (character2.PONTOS > character1.PONTOS) {
        console.log(chalk.bgCyan.greenBright.bold (`🎉 O grande vencedor é ${character2.NOME}! Parabéns! 🎉`));
    } else {
